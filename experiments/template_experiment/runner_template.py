...
import config
import os
from tracing_models.template import Template
...
# Check for cuda and user-specified param for GPU
...
# specify if entire dataset should be loaded to cuda all at once (only possible for smaller datasets)
...
# load the data produced by human learners
learner_data, experiment_vars, training_data = eut.load_learner_data(args.data_folder, get_training_data=True,
                                                                     device='cpu')
# stores a dict with each key describing the information of interest
# and the mask to pull it from the last dimension of the tensor learner_data
mask_dict = experiment_vars['mask_dict']

# stores the indices for train, val, and test partition
split_dict = experiment_vars['split_dict']

# create a namespace variable for the input params of the experiment (things like num_classes, num_learners, etc...)
evars = argparse.Namespace(**experiment_vars['input_params'])

# load the images and labels used to train the human learners -- has the keys "input_data" and "supervision"
...
if not args.continue_training:
    ...
    # create a feature extractor for the model
    ...
    if args.feature_extractor_path != 'none':
        ...
        # transform the images into the required format for the feature extractor
        # (ie. resnet pretrained on imagenet expects normalized images of size 224x224x3
        ...
        # if creating one from scratch, match the dataset to a feature extractor with the appropriate input size
    ...
    model = Template(response_dimension=(evars.num_supervised_queries, evars.num_classes),
    ...
    # If the user has specified to continue training, search for the folder of the trained models, then load the last
    # model checkpoint and continue training from there.
...
# pass the input_data_dict, mask_dict, split_dict, learner_data and load_method to a formatting function defined by
# the tracing model
...
# if the user specifies to freeze the weights of the feature extractor
if args.freeze_feature_extractor:
    model.set_feature_extractor_trainable(False)

# specify learning rates for different parts of the network
optimizer = torch.optim.Adam([
    {'params': model.network.parameters(), 'lr': args.lr},
...
# specify variables that will be filled during training the model
loss = None
results = {'acc': [],
...
def train():
    ...
    for c, batch in enumerate(batch_indices):
        ...
        # use the experiment utilities function to turn a list of indices into a batch
        data_batch = eut.index_dict_by_batch_indices(data, bli, load_method[1])
        out = model(**data_batch)  # run model on batch
...
# save checkpoint models at these epochs
checkpoint_list = [0, 200, 400]

# At any point you can hit Ctrl + C to break out of training early and the model will still save.
try:
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # Save the model if the acc is the best we've seen so far.
        ...
        if args.early_stopping:
            if val_loss > best_val_loss:
                ...
                val_loss_increasing_counter = 0
            # change this threshold to make early stopping more or less aggressive
