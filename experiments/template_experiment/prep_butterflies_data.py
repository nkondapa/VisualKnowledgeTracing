...
data_source = 'butterflies'
exp_name = 'butterflies'
...
learner_data, experiment_vars = convert_user_data_to_tensor_kfold(db_dict, 5, exp_name, valid_session_params,
                                                                  split_num_folds=(21, 4, 5),
                                                                  kfold=5, seed=523, return_train_and_test=True)

for i, evar in enumerate(experiment_vars):
    experiment_name_ = exp_name + '_fold' + str(i)
    print(evar['input_params']['sequence_length'])
    save(learner_data, evar, '', 'butterflies/' + f'{experiment_name_}_train_test')
    train_seq_len = evar['input_params']['train_sequence_length']
    evar['input_params']['sequence_length'] = train_seq_len
    save(learner_data[:, :train_seq_len], evar, '', 'butterflies/' + f'{experiment_name_}')
