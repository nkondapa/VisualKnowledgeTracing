...
from tracing_models.dkt_translation import DKTTranslation
...
# ARCHITECTURE PARAMS
parser.add_argument('--feature-dimension', type=int, default=64,
                    help='the dimensionality of the feature space')
parser.add_argument('--hidden-state-dimension', type=int, default=100,
...
parser.add_argument("--cell_type", dest="cell_type", default="lstm", type=str, required=False)
parser.add_argument("--gamma", dest="gamma", default=0.05, type=float, required=False)
parser.add_argument("--reg1", dest="reg1", default=0.00, type=float, required=False)
parser.add_argument("--reg2", dest="reg2", default=0.0, type=float, required=False)
parser.add_argument("--num_worker", dest="num_worker", default=0, type=int, required=False)
...
args.num_skill = evars.num_classes
...
if not args.continue_training:
    if args.feature_extractor_path != 'none':
        ...
        fe_model = None
    model = DKTTranslation(embed_dim=evars.num_classes * 3,
                           input_dim=0,
                           hidden_dim=args.hidden_state_dimension,
                           layer_num=args.num_layers,
                           output_dim=args.num_skill,
                           gamma=args.gamma,
                           reg1=args.reg1,
                           reg2=args.reg2,
...
def evaluate(_data):
    ...
    _num_learners = _data['qa'].shape[0]
    model.eval()
    total_loss = 0.
    ...
    with torch.no_grad():
        ...
        for batch in range(num_batches):
            ...
            results_mat[_bli, :] = (out['probs'].argmax(-1) == data_batch['responses_t'].argmax(-1).squeeze(-1)).type(
                torch.float).reshape(bsz, evars.sequence_length, evars.num_queries).detach().cpu()

            loss = model.loss_function(out, **data_batch)
            total_loss += loss.item()

    return total_loss / (_num_learners / batch_size), results_mat
...
def train():
    ...
    for c, batch in enumerate(batch_indices):
        ...
        results_mat[bli, :] = (out['probs'].argmax(-1) == data_batch['responses_t'].argmax(-1).squeeze(-1)).type(
            torch.float).reshape(bsz, evars.sequence_length, evars.num_queries).detach().cpu()
...
try:
    ...
    val_loss, val_seq_acc = evaluate(formatted_data['val'])
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.4f} | '
              ' acc {:0.4f} '.format(epoch, (time.time() - epoch_start_time),
                                     val_loss, torch.mean(val_seq_acc).item()))
...
# Run on test data.
test_loss, test_seq_acc = evaluate(formatted_data['test'])
ut.file_utilities.save_model(
