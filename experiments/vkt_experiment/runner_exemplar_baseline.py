...
from observer_models.baseline import ExemplarBaseline
...
if not args.continue_training:
    if args.feature_extractor_path != 'none':
        ...
        if 'greebles' in args.experiment_data_specifier:
            ...
            fe_model = 'cnn144_1ch'
    model = ExemplarBaseline(response_dimension=(evars.num_supervised_queries, evars.num_classes),
    ...
    # args.experiment_name = sys.argv[0].split('/')[-1].split('_')[-1].split('.py')[0] + '_' + 'gaussian_blob4_4x1'
    ...
    # args.experiment_name = sys.argv[0].split('/')[-1].split('_')[-1].split('.py')[0] + '_' + args.experiment_data_name
...
optimizer = torch.optim.Adam([
    {'params': model.c, 'lr': args.lr},
    {'params': model.gamma, 'lr': args.lr},
...
def train():
    ...
    for c, batch in enumerate(batch_indices):
        ...
        loss, loss_vals = model.loss_function(out, data_batch['responses_t'])
        loss.backward()
        if torch.isnan(model.feature_extractor.feature_extractor[0].weight.grad).any():
            print()
        optimizer.step()
