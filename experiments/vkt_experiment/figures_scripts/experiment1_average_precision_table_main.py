...
out_path = f'../figures/{tag_name}/main_paper_tables/'
...
script_name_to_paper_name = {
    ...
    'runner_timestep_baseline': '$\phi_{static\_time}$',
    'runner_direct_response_model1v3': '$\phi_{direct}$',
    'runner_classifier_model1v2': '$\phi_{cls_pred}$',
}
...
for script_name in average_dict:
    for dataset_name in average_dict[script_name]['dataset_names']:
        ...
        for kk in entry['train_seq_AP']:
            ...
            entry['train_seq_AP'][kk] = f'{mean:0.2f}'
        for kk in entry['test_seq_AP']:
            mean = np.mean(entry['test_seq_AP'][kk])
            std = np.std(entry['test_seq_AP'][kk])
            entry['test_seq_AP'][kk] = f'{mean:0.2f}'

refactor_entries = {}
c = 0
for script_name in script_names:
    if script_name not in script_name_to_paper_name:
        continue
    for dataset_name in average_dict[script_name]['dataset_names']:
