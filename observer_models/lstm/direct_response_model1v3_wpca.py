...
class LSTMObserverModel(nn.Module):
    ...
    def __init__(self, response_dimension, input_dimension, out_dimension, seq_len, pretrained_feature_extractor=None,
                 ...
                 **kwargs):
        super(LSTMObserverModel, self).__init__()
        ...
        self.network = nn.LSTM(
            input_size=np.prod(self.response_dimension) * 2 + np.prod(self.input_dimension) + out_dimension[0],
            hidden_size=self.hidden_state_dimension,
            batch_first=True,
            num_layers=num_layers, dropout=dropout)

        self.transform_hidden_state_to_response = nn.Sequential(
            nn.PReLU(),
            nn.Linear(self.hidden_state_dimension, np.prod(self.out_dimension)),
            nn.PReLU(),
            nn.Linear(np.prod(self.out_dimension), self.out_dimension[0])
        )
    ...
    def forward(self, responses_t, data_t, teaching_signal_t, **kwargs):
        ...
        pc_corr = ((responses_t.squeeze() == 1) & (teaching_signal_t == 1)).type(torch.float).cumsum(
            1)  # get correct per class
        pcc = teaching_signal_t.cumsum(1)  # get all per class
        pca = pc_corr / pcc  # compute per class accuracy
        pca[torch.isnan(pca)] = -1  # replace nans (unknown acc with neg 1)
        pca = pca.unsqueeze(-2)

        # offset with unknown acc pad (response information should be delayed by one step)
        pca = torch.cat([torch.ones_like(pca[:, [0]]) * -1, pca], dim=1)[:, :-1]
        ...
        for t in range(self.seq_len):
            ...
            if t == 0:
                x = torch.cat([feats[:, t].flatten(start_dim=1), pca[:, t].flatten(start_dim=1),
            ...
            response = self.transform_hidden_state_to_response(out)
        ...
        return dict(hidden_states=hidden_states, cell_states=cell_states,
                    last_cell_state=out_c, last_hidden_state=hidden_state,
                    init_responses=init_responses, feats=feats.detach(),
                    last_feat=feats[:, [-1]], per_class_accuracies=pca,
