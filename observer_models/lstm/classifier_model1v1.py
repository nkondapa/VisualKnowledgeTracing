...
class LSTMObserverModel(nn.Module):
    ...
    def forward(self, responses_t, data_t, teaching_signal_t, **kwargs):
        ...
        teaching_signal_pad = torch.zeros_like(teaching_signal_t[:, 0])

        for t in range(self.seq_len + 1):

            if t == 0:
                x = torch.cat([feats_pad.flatten(start_dim=1), teaching_signal_pad.flatten(start_dim=1),
                               response_pad.flatten(start_dim=1)], dim=-1).unsqueeze(1)
                out, (hidden_state, out_c) = self.network(x)
            else:
                x = torch.cat([feats[:, t - 1].flatten(start_dim=1), teaching_signal_t[:, t - 1].flatten(start_dim=1),
        ...
        return dict(hyperplanes=hyperplanes, bias=bias, init_hyperplanes=init_hyperplanes,
                    cell_states=cell_states, hidden_states=hidden_states,
                    last_cell_state=out_c, last_hidden_state=hidden_state,
                    feats=feats.detach(), last_feat=feats[:, [-1]],
                    last_response=responses_t[:, [-1]], last_teaching_signal=teaching_signal_t[:, [-1]],
                    responses=responses, init_responses=init_responses)
