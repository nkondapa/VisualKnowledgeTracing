...
class LSTMObserverModel(nn.Module):
    ...
    def forward(self, responses_t, data_t, teaching_signal_t, **kwargs):
        ...
        feats_pad = torch.zeros(size=(bsz, self.out_dimension[1]), device=device)

        for t in range(self.seq_len):

            if t == 0:
                x = torch.cat([feats_pad.flatten(start_dim=1), pca[:, t].flatten(start_dim=1),
                ...
                x = torch.cat([feats[:, t - 1].flatten(start_dim=1), pca[:, t].flatten(start_dim=1),
                               teaching_signal_t[:, t].flatten(start_dim=1),
