...
class PrototypeBaseline(nn.Module):
    ...
    def __init__(self, response_dimension, input_dimension, out_dimension, seq_len, pretrained_feature_extractor=None,
        ...
        super(PrototypeBaseline, self).__init__()
    ...
    def forward(self, responses_t, data_t, teaching_signal_t, **kwargs):
        ...
        responses = []
        prototypes = torch.zeros(size=(responses_t.shape[0], responses_t.shape[-1], feats.shape[-1])).to(feats.device)
        prototype_counts = torch.ones(size=(responses_t.shape[0], responses_t.shape[-1], 1), dtype=torch.float).to(
            feats.device)
        teaching_labels = teaching_signal_t.type(torch.bool)
        for t in range(responses_t.shape[1]):
            curr = feats[:, t]
            average_prototypes = prototypes / prototype_counts.clone()
            dist = torch.sqrt(torch.sum((curr - average_prototypes) ** 2, dim=-1)).unsqueeze(-2)
            score = torch.exp(self.c * dist * -1)
            responses.append(score)

            # update prototype
            prototypes[teaching_labels[:, t]] = prototypes[teaching_labels[:, t]] + feats[:, t, 0]
            prototype_counts[teaching_labels[:, t]] = prototype_counts[teaching_labels[:, t]] + 1
        ...
        return dict(init_responses=init_responses, feats=feats.detach(),
                    responses=responses, prototypes=prototypes, prototype_counts=prototype_counts)
