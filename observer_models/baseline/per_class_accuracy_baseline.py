...
class PerClassAccuracyBaseline(nn.Module):
    ...
    def __init__(self, response_dimension, input_dimension, out_dimension, seq_len, pretrained_feature_extractor=None,
        ...
        super(PerClassAccuracyBaseline, self).__init__()
        ...
        self.conditional_transform = nn.Sequential(
            nn.Linear(self.out_dimension[1] + self.out_dimension[0], 32),
            nn.PReLU(),
            nn.Linear(32, 32),
            nn.PReLU(),
        )
        self.classifier = nn.Linear(self.out_dimension[1] + self.out_dimension[0], self.out_dimension[0])
        self.classifier = nn.Linear(32, self.out_dimension[0])
    ...
    def forward(self, responses_t, data_t, teaching_signal_t, **kwargs):
        ...
        pca_feats = torch.cat([feats, pca], dim=-1)
        _out = self.conditional_transform(pca_feats)
        responses = self.classifier(_out)
        init_responses = responses[:, 0]
        return dict(init_responses=init_responses, feats=feats.detach(),
                    responses=responses, per_class_accuracies=pca)
